...
def load_config_bits(run_dir: Path) -> tuple[str, str]:
    ...
    model_bits = f"a={actor}; c={critic}; r={router}; e={executor}"
    runtime_bits = "; ".join(
        [
...
def collect_rows(runs_root: Path) -> tuple[list[dict], list[dict]]:
    ...
    run_dirs = [
        ...
        if path.is_dir() and path.name not in EXCLUDED_RUN_NAMES
    ]
    ...
    for run_dir in run_dirs:
        ...
        note_bits = [f"任务目录={len(task_dirs)}", f"已完成={finished}", f"成功={success_count}"]
